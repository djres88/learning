...
def slippery_numbers(n)
  ...
  until slipperies_to_n.length == n
    if is_slippery?(count)
      slipperies_to_n << count
    end
    count += 1
  end
  slipperies_to_n
...
def magic_numbers(count)
  ...
  until first_n_magix.length == count
    if is_magic_number?(idx)
      first_n_magix << idx
    end
    idx += 1
  end
...
def is_magic_number?(num)
  digits = num.to_s
  sum = 0
  digits.each_char { |fig| sum += fig.to_i }
  if sum == 7
    return true
  else
    return false
